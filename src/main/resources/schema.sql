...
CREATE TABLE `player_base` (
  ...
  PRIMARY KEY (`id`),
  KEY `FKpr3gkjjir6v97s48o22a440wp` (`country_name`),
  KEY `FKisv5s4da0jiifag5aau44kppq` (`team_name`),
  CONSTRAINT `FKisv5s4da0jiifag5aau44kppq` FOREIGN KEY (`team_name`) REFERENCES `team_base` (`name`),
  CONSTRAINT `FKpr3gkjjir6v97s48o22a440wp` FOREIGN KEY (`country_name`) REFERENCES `country_base` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `positions_base` (
  ...
  CONSTRAINT `FKog9k5jfa074aqmkekvoseus6y` FOREIGN KEY (`player_id`) REFERENCES `player_base` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*player_attributes_base*/

CREATE TABLE `player_attributes_base` (
  `aggression` decimal(19,2) DEFAULT NULL,
  `bravery` decimal(19,2) DEFAULT NULL,
  `dribbling` decimal(19,2) DEFAULT NULL,
  `first_touch` decimal(19,2) DEFAULT NULL,
  `jumping` decimal(19,2) DEFAULT NULL,
  ...
  `reflexes` decimal(19,2) DEFAULT NULL,
  PRIMARY KEY (`player_id`),
  CONSTRAINT `FKkbvs3tnkyl5aqlwbwdp8path9` FOREIGN KEY (`player_id`) REFERENCES `player_base` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
